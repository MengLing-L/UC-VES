...
void COCO_Framework_Prove(COCO_Framework_PP &pp, 
    ...
    Encrypt_witNess_or_Encrypt_signature_Prove(pp.pp, instance.instance, witness.witness, chl, proof.proof, EK, EK1);

    BN_vec_free(split_each_4bytes_m);
    BN_vec_free(each_4bytes_m_beta);
}

bool COCO_Framework_Verify(COCO_Framework_PP &pp, 
                            COCO_Framework_Instance &instance, 
                            BIGNUM *&chl,  
                            COCO_Framework_Proof &proof,
                            EC_POINT* &EK,
    ...
    return Encrypt_witNess_or_Encrypt_signature_Verify(pp.pp, instance.instance, chl, proof.proof, EK, EK1);
}
