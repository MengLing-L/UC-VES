...
#include "../COCO-framework/encrypt_witness_not_encrypt_random.hpp"
...
void Encrypt_witNess_or_Encrypt_signature_Prove(Encrypt_witNess_or_Encrypt_signature_PP &pp, 
    ...
    BN_random(proof.chl1);

    //chl1 = BN_bn2string(tmp_chl1);

    Simulation_Encrypt_Signature_Simulate_Proof(pp.sim_sig_pp, instance.sim_sig_instance, proof.chl1, proof.sim_sig_proof, EK);

    Witness_Encryption_AndR_Commit(pp.enc_witness_pp, instance.enc_witness_instance, witness.enc_witness_witness, proof.enc_witness_proof, EK, EK1);

    //BIGNUM *e = BN_new();
    //BN_random(e);

    BN_sub (proof.chl0, e, proof.chl1);

    Witness_Encryption_AndR_Res(pp.enc_witness_pp, instance.enc_witness_instance, witness.enc_witness_witness, proof.chl0, proof.enc_witness_proof);
...
bool Encrypt_witNess_or_Encrypt_signature_Verify(Encrypt_witNess_or_Encrypt_signature_PP &pp, 
                            ...
                            EC_POINT* &EK,
                            EC_POINT* &EK1){
    return Simulation_Encrypt_Signature_Verify(pp.sim_sig_pp, instance.sim_sig_instance, proof.chl1, proof.sim_sig_proof, EK)
            && Witness_Encryption_AndR_Verify(pp.enc_witness_pp, instance.enc_witness_instance, proof.chl0, proof.enc_witness_proof, EK, EK1);
